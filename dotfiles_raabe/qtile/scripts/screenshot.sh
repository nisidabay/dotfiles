...
case $choice in
    $option1)
        ...
        notify-send "Screenshot created [$DIR]" "Mode: Selected window"
    ;;
    $option2)
        scrot $DIR$NAME -s -e 'xclip -selection clipboard -t image/png -i $f'
        notify-send "Screenshot created [$DIR]" "Mode: Selected area"
    ;;
    $option3)
        scrot $DIR$NAME -d 3 -e 'xclip -selection clipboard -t image/png -i $f'
        notify-send "Screenshot created [$DIR]" "Mode: Fullscreen"
    ;;
esac
