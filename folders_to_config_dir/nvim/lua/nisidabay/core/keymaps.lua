...
keymap.set('n', '<leader>.', [[:lua Change_cwd_to_current_file_dir()<CR>]], { silent = true, desc = 'Change_cwd_to_current_file_dir' })

-- Text surrounding and markdown
keymap.set('x', '<leader>vc', [[:s/\%V\(^.*$\)/`&`/g<CR>gv]], { silent = true, desc = 'Surround selected text with quotes' })
...
keymap.set('n', '<leader>gc', ':!gcc -Wall -Wextra -g -std=c11 -o %:r %<CR>', { silent = true, desc = 'Compile C code' })
keymap.set('n', '<leader>gcn', ':!gcc -Wall -Wextra -g -std=c11 -o %:r -lncurses %<CR>', { silent = true, desc = 'Compile ncurses C code' })
keymap.set('n', '<leader>gcs', ':!gcc -Wall -Wextra -g -std=c11 -o %:r -lnsqlite %<CR>', { silent = true, desc = 'Compile sqlite C library' })
...
-- Buffers
keymap.set('n', '<leader>b', [[:set nomore <Bar> :ls <Bar> :set more <CR>:b<leader>]], { silent = true, desc = 'Show buffers' })

keymap.set('n', '<leader>ws', ':w !sudo tee <C-R>=input("Save to file: ")<CR> > /dev/null<CR>', { silent = true, desc = 'Save buffer as sudo' })
keymap.set('n', '<leader>cab', [[:update <bar> %bd <bar> e# <bar> bd# <CR><CR>]], { silent = true, desc = 'Close all buffers but this one' })
